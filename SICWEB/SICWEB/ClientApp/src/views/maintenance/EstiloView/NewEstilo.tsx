...
import { getBrands, getCategory, getColor, getColors, getImage, getOriginCategories, getTallas, saveStyle } from 'src/apis/styleApi';
...
const NewEstilo: FC<NewEstiloProps> = ({
    ...
    useEffect(() => {
        imageData ! == null && window.setTimeout(() => {
            setReady(true);
        }, 300);
    ...
    const setDefaultData = () => {
        ...
        if(editID > -1) {
            ...
            getColor({id: fValues}).then(res => {
                setColors(res);
             }).catch(err => {
                setColors([]);
            });
            getCategory({id: fValues}).then(res => {
                setCategories(res)
             }).catch(err => {
                setCategories([]);
            });
